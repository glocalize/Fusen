...
const normComment = (r) => (r ? { ...r, guest: !!r.guest, ctx_modal: r.ctx_modal == null ? null : !!r.ctx_modal } : null);
...
export async function addComment(db, m) {
  await db
    ...
    .run();
  return normComment({ ...m, guest: b(m.guest), resolved_by: m.resolved_by ?? null, ctx_modal: bn(m.ctx_modal ?? null) });
}
