...
app.get("/", (c) => {
  ...
  if (!getUser(c.req.raw)) return c.redirect("/login");
...
app.get("/s/:token", async (c) => {
  const canvas = await db.getCanvasByToken(c.env.DB, c.req.param("token"));
  if (!canvas) return c.text("共有リンクが無効です", 404);
  if (!getUser(c.req.raw)) return c.redirect(`/login?guest=1&next=${encodeURIComponent("/p/" + canvas.id)}`);
